...
import com.example.amnyamai.data.model.TaskStatus
...
class ResultViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun load(meetingId: String) {
        ...
        viewModelScope.launch {
            repository.pollResult(meetingId).fold(
                onSuccess = { meeting ->
    ...
    fun acceptTask(task: Task) {
        ...
        viewModelScope.launch {
            repository.saveTaskToCalendar(task)
        }
        advance(state)
    }
    ...
    fun updateTaskTitle(index: Int, newTitle: String) {
        ...
        updated[index] = updated[index].copy(title = newTitle)
